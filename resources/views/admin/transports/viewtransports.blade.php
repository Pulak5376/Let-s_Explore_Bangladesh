...
@section('content')
<div class="transport-content-wrapper">
    <div class="filter-section">
        <button id="showAll" class="filter-btn {{ !request('type') ? 'active' : '' }}">All Transports</button>
        <button id="showBus" class="filter-btn {{ request('type') == 'bus' ? 'active' : '' }}">Bus Transports</button>
        <button id="showTrain" class="filter-btn {{ request('type') == 'train' ? 'active' : '' }}">Train Transports</button>
    </div>
    
    <div class="search-section">
        <form action="{{ route('admin.transports.search') }}" method="GET" class="search-form">
            <input type="text" name="search" placeholder="🔍 Search Transports by Name, Type, or Route..." value="{{ request('search') }}">
            @if(request('type'))
    ...
    <div class="table-wrapper">
        <table id="transportsTable" class="transport-table">
            <thead>
                <tr>
                    <th>ID</th>
                    <th>Name</th>
            ...
            <tbody>
                @foreach ($transports as $transport)
                    <tr data-type="{{ $transport->type }}">
                        <td data-label="ID">{{ $transport->id }}</td>
                        <td data-label="Name">{{ $transport->name }}</td>
                        <td data-label="Type">{{ ucfirst($transport->type) }}</td>
                        <td data-label="Route From">{{ $transport->route_from }}</td>
                        <td data-label="Route To">{{ $transport->route_to }}</td>
                        ...
                        <td data-label="Delete">
                            <form id="delete-form-{{ $transport->id }}" action="{{ route('admin.transports.destroy', $transport->id) }}" method="POST"
                                style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="delete-btn" onclick="showDeleteModal({{ $transport->id }}, '{{ $transport->name }}', '{{ $transport->type }}')">
...
<div id="deleteModal" class="modal-overlay">
    <div class="modal-container">
        ...
        <div class="modal-body">
            <p>Do you want to delete this transport?</p>
            <div class="transport-details">
                <div class="detail-item">
                    <span class="label">Transport Name:</span>
                    <span id="transportName" class="value"></span>
                </div>
                <div class="detail-item">
                    <span class="label">Transport Type:</span>
                    <span id="transportType" class="value"></span>
                </div>
            </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.getElementById('showAll').addEventListener('click', function() {
                ...
                let url = "{{ route('admin.transports.viewtransports') }}";
                if (searchParam) {
            ...
            document.getElementById('showBus').addEventListener('click', function() {
                const searchParam = new URLSearchParams(window.location.search).get('search');
                let url = "{{ route('admin.transports.viewtransports') }}?type=bus";
                if (searchParam) {
                    url += "&search=" + encodeURIComponent(searchParam);
                }
                window.location.href = url;
            ...
            document.getElementById('showTrain').addEventListener('click', function() {
                const searchParam = new URLSearchParams(window.location.search).get('search');
                let url = "{{ route('admin.transports.viewtransports') }}?type=train";
                if (searchParam) {
                    url += "&search=" + encodeURIComponent(searchParam);
                }
                window.location.href = url;
        ...
        function showDeleteModal(transportId, transportName, transportType) {
            currentTransportId = transportId;
            document.getElementById('transportName').textContent = transportName;
            document.getElementById('transportType').textContent = transportType.charAt(0).toUpperCase() + transportType.slice(1);
        ...
        function confirmDelete() {
            if (currentTransportId) {
                document.getElementById('delete-form-' + currentTransportId).submit();
            }
        }
...
<style>
    ...
    .transport-table tbody tr:nth-child(odd) {
