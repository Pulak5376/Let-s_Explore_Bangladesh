  ...
  <div class="train-booking-container">
      ...
      <form action="{{ route('train.search') }}" method="POST">
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            ...
            <input type="text" id="to" name="to" value="{{ old('to') }}" placeholder="e.g. Rajshahi" required>
          </div>
          <div class="form-group">
    ...
    <div class="results-container">
      ...
      <div class="train-results">
        @foreach ($transports as $train)
        <div class="train-card">
          <div class="train-header">
            <span class="operator">{{ $train->name ?? 'Train Service' }}</span>
            <span class="train-type">{{ ucfirst($train->type) }}</span>
          </div>
          <div class="journey-details">
            <div class="route">
              <span class="from">{{ $train->route_from }}</span>
              <span class="route-arrow">→</span>
              <span class="to">{{ $train->route_to }}</span>
            </div>
            <div class="schedule">
              <span class="time">Departure: {{ $train->departure_time }}</span>
            </div>
          </div>
          <div class="train-footer">
            <div class="train-info">
              <span class="fare">৳{{ $train->price }}</span>
              <span class="seats">{{ $train->total_seats }} seats available</span>
            </div>
            <form action="{{ url($train->type.'/book') }}" method="POST" style="width:100%;" class="train-book-form">
              @csrf
              <input type="hidden" name="transport_id" value="{{ $train->id }}">
              <div class="booking-controls">
                <div class="seat-selection">
                  <label for="seats_{{ $train->id }}">Seats:</label>
                  <input type="number" id="seats_{{ $train->id }}" name="seats_booked" value="1" min="1" max="{{ $train->total_seats }}">
                </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.train-book-form').forEach(function(form) {
      ...
      if (showBtn && passengerInfo) {
        showBtn.addEventListener('click', function(e) {
          ...
          document.querySelectorAll('.train-book-form .passenger-info').forEach(function(pi) {
